...
//            'drop' : true
    ...
    onRender: function(ct, position){
        ...
        // listviewadd parent to scollbar
        //var divCont = new Ext.Layer({ cls: "flexcroll" });
        
        //console.info( divCont );
        ...
        var fs = this.fs = new Ext.form.FieldSet({
            ...
            height: this.height,
            width: this.width,
            style: this.style || {padding:'0'},
            tbar: this.tbar,
            bbar: this.bbar,
            fbar: this.fbar,
            bodyStyle: this.bodyStyle || 'overflow: auto;padding:5px 0;'
             
        });

        //console.info( fs.bwrap.firstElementChild );
        ...
        this.view = new Ext.ListView({
            ...
            hideHeaders: true,
            simpleSelect: this.simpleSelect || false,
            loadingText : this.loadingText || 'Wait ...'
//            plugins : [
//              
//            ]
            
            //tooltip:'{' + this.displayField + '}'
            
        });
        
        // display number of selected items
//        this.view.on('selectionchange', function(view, nodes) {
//          var l = nodes.length;
//          var s = l != 1 ? 's' : '';
//          
//          var title = fs.title.indexOf(' (',0) !== -1 ? fs.title.substr(0, fs.title.indexOf(' (',0)) : view.origTitle = fs.title ;
//          var title = (l === 0) ? view.origTitle : title + ' ('+l+' item'+s+' sel.)';
//          
//          fs.setTitle(title);
//        });
        
//        this.view.on('mouseenter', function(view, idx, node, e){
//          console.info(view);
//          console.info(idx);
//          console.info(node);
//          console.info(e);
////          var l = nodes.length;
////          var s = l != 1 ? 's' : '';
////          fs.setTitle('Simple ListView ('+l+' item'+s+' selected)');
//        });
...
//        }
    ...
    onViewClick: function(vw, index, node, e) {
        // put this logic here to select to process when node 0 'all' is selected
        var selectionsArray = this.view.getSelectedIndexes();
        
        if(selectionsArray.indexOf(0) !== -1) {
          if(selectionsArray.length > 1 && index === 0) {
            this.setValue('0');
          } else if(index !== 0) {
            this.view.deselect(0);
          }
        }
    ...
    reset : function() {
        ...
        this.setValue('');
        this.fireEvent('afterreset', this);
    },
